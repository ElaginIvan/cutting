document.addEventListener('DOMContentLoaded', () => {
    ...
    const pdfBlackAndWhiteCheckbox = document.getElementById('settings-pdf-bw');
    const pdfJpegQualitySlider = document.getElementById('settings-pdf-jpeg-quality');    
    const magnifierEnabledCheckbox = document.getElementById('settings-magnifier-enabled');
    const themeSwitcher = document.getElementById('settings-theme-switcher');
    ...
    form.addEventListener('submit', (e) => {
        ...
        if (!name || sizes.length === 0) {
            ConfirmationModal.show({
                title: 'Ошибка',
                message: 'Пожалуйста, заполните все поля: "Сортамент" и "Типоразмеры".',
    ...
    listContainer.addEventListener('click', (e) => {
        ...
        if (target.classList.contains('btn-delete-assortment')) {
            ...
            ConfirmationModal.show({
                title: 'Удалить сортамент?',
                message: `Вы уверены, что хотите удалить "${name}"? Это действие нельзя будет отменить.`,
                onConfirm: () => {
        ...
        deficitLengthInput.addEventListener('change', (e) => {
            ...
            if (lengthsStr === '') { // Если поле очищено
                ...
                ConfirmationModal.show({
                    ...
                    message: 'Пожалуйста, введите список длин через запятую, например: 6000, 12000. Длины должны быть положительными числами.',
                    hideConfirmButton: true,
                    cancelText: 'Закрыть'
                });
    ...
    function initializePrintSettings() {
        ...
        pdfBlackAndWhiteCheckbox.checked = settings.pdfIsBlackAndWhite;
        ...
        pdfBlackAndWhiteCheckbox.addEventListener('change', (e) => DB.updateSetting('pdfIsBlackAndWhite', e.target.checked));
    ...
    // --- Логика для настроек оформления ---
    function initializeAppearanceSettings() {
        const settings = DB.getSettings();
        themeSwitcher.checked = settings.theme === 'dark';
        magnifierEnabledCheckbox.checked = settings.magnifierEnabled;

        themeSwitcher.addEventListener('change', (e) => {
            const newTheme = e.target.checked ? 'dark' : 'light';
            DB.updateSetting('theme', newTheme);
            // Отправляем событие, чтобы main.js мог применить тему немедленно
            window.dispatchEvent(new CustomEvent('themeChanged', {
                detail: { theme: newTheme }
            }));
        });

        magnifierEnabledCheckbox.addEventListener('change', (e) => {
            const isEnabled = e.target.checked;
            DB.updateSetting('magnifierEnabled', isEnabled);
            window.dispatchEvent(new CustomEvent('magnifierSettingChanged', {
    ...
    const workModeLabel = document.querySelector('label[for="settings-work-mode"]');

    function initializeWorkModeSettings() {
        const settings = DB.getSettings();
        workModeCheckbox.checked = settings.workMode === 'warehouse';
        updateWorkModeLabel(workModeCheckbox.checked);

        workModeCheckbox.addEventListener('change', (e) => {
            const newMode = e.target.checked ? 'warehouse' : 'simple';
            updateWorkModeLabel(e.target.checked);
            DB.updateSetting('workMode', newMode);
            window.dispatchEvent(new CustomEvent('workModeChanged', { detail: { mode: newMode } }));
        });
    }

    function updateWorkModeLabel(isWarehouse) {
        workModeLabel.textContent = isWarehouse
            ? 'Складской режим (включен)'
            : 'Включить складской режим';
    }

    // --- Логика для UI аккордеона ---
    const accordionHeaders = document.querySelectorAll('.accordion-header');
    // Оборачиваем содержимое каждого аккордеона для новой CSS-анимации
    ...
    accordionHeaders.forEach(header => {
        header.addEventListener('click', function () {
            ...
            if (content.id === 'instructions-content' && !innerContent.dataset.loaded) {
                fetch('instructions.html')
                    .then(response => response.ok ? response.text() : Promise.reject('Failed to load'))
                    .then(html => { innerContent.innerHTML = html; innerContent.dataset.loaded = 'true'; })
                    .catch(() => { innerContent.innerHTML = '<p class="text-muted" style="padding: 1rem;">Не удалось загрузить инструкцию.</p>'; });
            }
        });
    });
